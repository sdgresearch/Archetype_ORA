"""Figure 4: Official-postcode archetype geography and 12-PC profiles with the accepted 4-km local views.

Run: python Figure4_residential_archetypes.py
Optional: --output-dir my_figures --csv Figure4_residential_archetypes.csv
...
FIGURE_NUMBER = 4
FIGURE_NAME = 'Figure4_residential_archetypes'
OUTPUT_RELATIVE_STEM = 'Figure1_Residential_urban_exposome_contexts'
...
def figure1(post, domain_indicators, pc_scores, pc_dictionary, arch, extent, boundary):
    ...
    zoom_qa["pass"] = bool(
        zoom_qa["postcode_rows"] == EXPECTED_POSTCODES and zoom_qa["zoom_regions"] == 4
...
ENTRY = '''
"""Reproduce Figure 4 from compact processed residential and PC tables."""
from pathlib import Path
import sys
import pandas as pd
...
import plot_residential_green_cooling_paper_figures as renderer

renderer.ROOT = HERE.parent
renderer.BASE = HERE / 'data/base'
renderer.ANALYSIS = HERE / 'data/analysis'
renderer.OUT = HERE / 'output'
renderer.SOURCE = renderer.OUT / 'source_data'
renderer.FIGURE1_SHOW_NAVIGATIONAL_HEADINGS = False
renderer.EXPECTED_POSTCODES = 94_076
renderer.planning_boundary = lambda crs: renderer.gpd.read_file(HERE.parent / 'shared_data/MasterPlan2019PlanningAreaBoundaryNoSea.geojson').to_crs(crs)
post = pd.read_csv(renderer.ANALYSIS / 'postcode_cooling_production_and_service.csv')
domains = pd.read_csv(renderer.BASE / 'postcode_domain_indicators.csv')
pc_scores = pd.read_csv(renderer.ANALYSIS / 'pc_cooling_associations/postcode_balanced_pca_component_scores.csv')
pc_dictionary = pd.read_csv(renderer.ANALYSIS / 'pc_cooling_associations/pca_component_dictionary.csv')
arch, extent, crs, _ = renderer.read_raster('archetype_surface_100m.tif')
boundary = renderer.planning_boundary(crs)
renderer.figure1(post, domains, pc_scores, pc_dictionary, arch, extent, boundary)
'''
