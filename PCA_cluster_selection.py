"""Figure 2: Official 94,076-postcode five-domain PCA and K=1--20 diagnostics; no reclustering.

Run: python Figure2_PCA_cluster_selection.py
Optional: --output-dir my_figures --csv Figure2_PCA_cluster_selection.csv
Produces PNG/TIFF and editable-text PDF/SVG without overwriting an existing render.
The one matching CSV contains named processed tables and explicit raster/geometry
values, never a pre-rendered image. No original project paths are required.
The plotting package is not a full upstream analysis or a public data deposit.
Confirm third-party redistribution permissions before publication.
"""
...
# Frozen, transparent plotting source sections.
RENDERERS = {}
...
def draw_internal_panel(fig: plt.Figure, spec, ktable: pd.DataFrame) -> None:
    ...
    for i, (column, ylabel, threshold, ylim) in enumerate(metrics):
        ...
        selected_k = int(ktable.loc[ktable["selected_primary"].astype(bool), "k"].iloc[0])
        optimum_k = int(ktable.loc[ktable["statistical_optimum"].astype(bool), "k"].iloc[0])
        highlights = [(selected_k, PRIMARY, True)]
        if optimum_k != selected_k:
            highlights.append((optimum_k, INTERNAL, False))
        for kval, colour, filled in highlights:
    ...
    selected_k = int(ktable.loc[ktable["selected_primary"].astype(bool), "k"].iloc[0])
    optimum_k = int(ktable.loc[ktable["statistical_optimum"].astype(bool), "k"].iloc[0])
    selected_value = ktable.loc[ktable["k"].eq(selected_k), "silhouette_mean"].iloc[0]
    top.annotate(f"K={selected_k} selected", xy=(selected_k, selected_value),
                 xytext=(max(2.1, selected_k - 0.95), 0.184), color=PRIMARY,
                 fontsize=5.9, fontweight="bold",
                 arrowprops=dict(arrowstyle="-", color=PRIMARY, lw=0.55))
    if optimum_k != selected_k:
        optimum_value = ktable.loc[ktable["k"].eq(optimum_k), "silhouette_mean"].iloc[0]
        top.annotate(f"K={optimum_k} internal optimum", xy=(optimum_k, optimum_value),
                     xytext=(optimum_k + 1.05, 0.184), color=INTERNAL,
...
def draw_viability_panel(ax: plt.Axes, ktable: pd.DataFrame) -> None:
    ...
    selected_k = int(ktable.loc[ktable["selected_primary"].astype(bool), "k"].iloc[0])
    optimum_k = int(ktable.loc[ktable["statistical_optimum"].astype(bool), "k"].iloc[0])
    highlights = [(selected_k, PRIMARY)]
    if optimum_k != selected_k:
        highlights.append((optimum_k, INTERNAL))
    for kval, colour in highlights:
        row = ktable.loc[ktable["k"].eq(kval)].iloc[0]
...
def draw_spatial_panel(ax: plt.Axes, ktable: pd.DataFrame) -> None:
    ...
    for (_, row), size in zip(spatial.iterrows(), sizes):
        ...
        elif kval == optimum_k and optimum_k != selected_k:
        ...
        if kval == selected_k:
            ax.text(row["mean_blocked_ari"] - 0.062,
                    ...
                    f"K={selected_k} primary", fontsize=5.8, color=edge, fontweight="bold")
        elif kval == optimum_k and optimum_k != selected_k:
            ax.annotate(
                f"K={optimum_k} internal optimum",
            ...
            dx, dy = label_offsets.get(kval, (dx, dy))
            ax.text(row["mean_blocked_ari"] + dx,
                    max(0.477, row["minimum_pooled_class_recall"] + dy),
    ...
    ax.set_ylim(0.47, 0.96)
    ax.set_xlabel("Mean blocked ARI")
    ax.set_ylabel("Weakest pooled cluster recall")

    selected = spatial.loc[spatial["k"].eq(selected_k)].iloc[0]
    text = (
        f"K={selected_k} retained\\n"
...
def main() -> None:
    ...
    if results["selected_primary_k"] != 4:
        raise ValueError(
            "This manuscript figure requires the validated K=4 reporting solution; "
            f"the supplied analysis selected K={results['selected_primary_k']}"
        )
    ...
    selected_k = int(results["selected_primary_k"])
    optimum_k = int(results["statistical_optimum_k"])
    selected = ktable.loc[ktable["k"].eq(selected_k)].iloc[0]
    internal = ktable.loc[ktable["k"].eq(optimum_k)].iloc[0]
    qa = {
        "figure_contract": {
            "core_conclusion": (
            ...
            "population": f"{int(results['cohort_postcodes']):,} official residential postcodes",
            "raw_variables": 124,
        ...
        "selection": {
            ...
            "statistical_internal_optimum_k": optimum_k,
            f"k{selected_k}": {
            ...
            f"k{optimum_k}_internal_optimum": {
